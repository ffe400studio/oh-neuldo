...
import { genId, CARD, FAB } from '../../constants'

export default function ResolutionManager({ goBack, resolutions, setResolutions, activeResolutionId, setActiveResolutionId }) {
  ...
  const save = () => {
    if (!text.trim()) return
    if (editId) {
      setResolutions(prev => prev.map(r => r.id === editId ? { ...r, text: text.trim() } : r))
    } else {
    ...
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', position: 'relative', backgroundColor: '#F5F2F3' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 16px 100px' }}>
        {resolutions.length === 0 ? (
          <div style={{ ...CARD, padding: '40px 20px', textAlign: 'center', marginTop: 8 }}>
            <p style={{ color: '#aaa', fontSize: 15 }}>아직 등록된 내용이 없어요 ✦</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {resolutions.map(r => {
              const isActive = r.id === activeResolutionId
              return (
                <div
                  key={r.id}
