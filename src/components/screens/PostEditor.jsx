...
export default function PostEditor({ goBack, topicId, postId, posts, setPosts }) {
  ...
  const save = () => {
    ...
    if (existing) {
      setPosts(prev => prev.map(p => p.id === postId ? { ...p, title, content, links } : p))
    } else {
      setPosts(prev => [...prev, {
        id: genId(),
        topicId,
            ...
            <div style={{ backgroundColor: '#fff', borderRadius: 14, padding: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                <button
                  ...
                  style={{ flex: 1, border: '1px solid #eee', borderRadius: 10, padding: '8px 0', background: '#fff', cursor: 'pointer', fontSize: 14 }}
                >
                  취소
                </button>
                <button
                  onClick={addLink}
                  style={{ flex: 1, backgroundColor: '#111', color: '#fff', border: 'none', borderRadius: 10, padding: '8px 0', cursor: 'pointer', fontSize: 14, fontWeight: 600 }}
                >
                  추가
                </button>
              </div>
